...
vi.mock("./agents.mirror", () => ({ getAgents: () => getAgentsMock() }));
vi.mock("./skills.mirror", () => ({ getSkillsMirror: () => getSkillsMock() }));
vi.mock("./mcp.mirror", () => ({ getMcp: () => getMcpMock() }));
vi.mock("./settings.service", () => ({ getSettings: () => getSettingsMock() }));

const search = await import("./user-search.service");
