...
describe("getSkill", () => {
  it("reads one SKILL.md fully including body and annex files", async () => {
    const { getSkill } = await import("../projects/project.skills?fresh=read");
  ...
  it("infers user scope for skills under ~/.claude", async () => {
    const { getSkill } = await import("../projects/project.skills?fresh=user");
    const skillFile = writeSkill(
  ...
  it("falls back to the directory name when frontmatter has no name", async () => {
    const { getSkill } = await import("../projects/project.skills?fresh=fallback");
    const skillFile = writeSkill(
  ...
  it("returns null when the SKILL.md is missing", async () => {
    const { getSkill } = await import("../projects/project.skills?fresh=missing");
